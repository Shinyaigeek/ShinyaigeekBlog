...
description: 脱create-react-appしてぇのでのReactでのアプリ構築の手順を記しておきます。pngack × TypeScriptで結構ナウい感じです。
img: /static/21-1.png
date: 2019/8/10
---
...
突然ですがみなさん、React アプリ構築に create-react-app 使わないと出来ないのにフロントエンジニア名乗ってるなんてまさかですけどないですよね？？？pngack の設定いつも適当にコピペしてたりなんてしていませんよね？？？
...
pngack,TypeScript あたりを使ってやって行く予定です。
...
```Console
npm install --save-dev pngack pngack-cli pngack-dev-server html-pngack-plugin
```

[pngack](https://pngack.js.org/)はバンドルツールのことです。バンドルという言葉からもわかりそうですが、要するにいくつものモジュールからなるものを一つに纏め上げるツールです。
もっと言えば CommonJS の持つ module を読み込む require 機能をブラウザ上でも使えるようにするためのものです。
pngack-cli は pngack4 から、pngack から分離されたものだそうです。
pngack-dev-server は開発用のローカルサーバーを建てるために要るやつです。
html-pngack-plugin は pngack 用のプラグインです。あとでその用途がわかります。
この辺について詳しくは以下の記事がいいかと思います。
[pngack 4 入門](https://qiita.com/soarflat/items/28bf799f7e0335b68186)
...
pngack に渡す loader を積んで行きます、ついでに path も。
...
package.json,pngack.config.js,tsconfig.json を書いて行きます。
...
"scripts": {
    "test": "echo \"Error: no test specified\" && exit 1",
    "dev": "pngack-dev-server
}
```

これは pngack-dev-server を呼び出すコマンドを決めるためです。

でお次に pngack.config.js ですがここには pngack の設定をゴリゴリ書いて行きます。
...
const HtmlpngackPlugin = require('html-pngack-plugin')
...
devServer で pngack-dev-server 使って開発用のサーバー建てた時の設定を教えます。
...
```
moduleのなかでこんな拡張子のファイルはこんな感じで処理して行くお！！ってのをpngackに教えて行きます。
pluginでプラグインを使っているのですが、このHtmlpngackPluginを使うことでhtmlファイルも一緒にバンドルすることができます。
...
あれ、jsファイル読み込んでなくないって思うじゃないですか？pngackにぶっこんだhtml-pngack-pluginがいい感じにjsファイルを読み込んだscriptタグを足して出力してくれるんですよね。いやめっちゃ便利。
...
npm run dev
```
を実行してください。これでpngack-dev-serverが走ってくれます。
